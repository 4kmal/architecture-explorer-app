...
### FYP Report collection

The **FYP Report / Laporan FYP** collection consolidates report material without changing either report-table renderer. **Project Planning / Perancangan Projek** contains the editable **Work Breakdown Structure / Struktur Pecahan Kerja**; **Report Tables / Jadual Laporan** retains the existing Data Dictionary and Use Case Specification resources and their copy-ready table behavior. The collection summary counts all three as report resources, while each subgroup identifies its diagram or table count accurately.

The Work Breakdown Structure is the canonical `1800 x 1350` (4:3) source for Figure 1.8. It contains one PetaKerja project root, five coordinated Agile phases, 21 report-ready work packages and 26 orthogonal arrow-free hierarchy connections. Simple mode displays the activities in the selected BM or English language. Code mode keeps every box and connector in the same position while replacing the activity text with repository evidence such as `src/MyPetaApp.ts`, `server/app.ts`, `supabase/migrations/`, `src/modes/jobs/`, test commands and documentation paths. View and Edit modes expose the same Simple/Code projection; Agent mode and component selection reuse the registered source, route, Supabase table and UI-hotspot metadata. The bilingual Figure 1.8 explanation remains visible in Implementation Details before and after a component is selected.

Only the local PetaKerja mark from `public/icon/android-chrome-192x192.png` is embedded in the root. The diagram contains no remote image dependency, gradient, emoji or decorative strip. For the FYP report, use the Simple/BM projection as the report-ready hierarchy; Code mode is an implementation-evidence view for verification and is not a second project plan. Regenerate and verify the canonical source, SVG preview and bundle with:

```powershell
python .\scripts\generate-fyp-work-breakdown-structure.py
python .\scripts\build-diagram-assets.py
python .\scripts\test-fyp-work-breakdown-structure.py
```
...
The V2 Domain and Implementation diagrams use `templates/Class Diagram Template.drawio` as their read-only UML style authority. The reviewed supervisor-report layout is also the default editable Core Domain source, so its class positions and component sizes are reused instead of returning to the earlier compact arrangement. The generator validates and copies its package, swimlane header, member, divider, relationship and multiplicity-label styles. Every class has a 46-pixel header with a 16-pixel title and 12-pixel stereotype, a 14-pixel typed attribute compartment, an optional 8-pixel divider and 14-pixel public-operation compartment, with variable heights rounded to the 10-pixel template grid. Parent classes keep the Explorer's stable component keys; child compartments use deterministic `/attributes`, `/divider` and `/operations` keys. Associations, dashed dependencies and hollow-diamond aggregations use orthogonal connector gutters with role labels and endpoint multiplicities.

To reapply the tracked default layout, or promote another reviewed Draw.io adjustment, run:

```powershell
python .\scripts\apply-class-diagram-default.py
python .\scripts\apply-class-diagram-default.py --adopt "C:\path\to\reviewed-class-layout.drawio"
python .\scripts\generate-v2-georouting-diagrams.py
```

The adoption step validates the `2650 x 2860` page, all 15 stable class keys and UML geometries before synchronising the template and default editor source. It changes only class-contained typography; package labels, relationship labels and the reviewed connector routing remain independent.
...
- `assets/editor/fyp-report/work-breakdown-structure.drawio` and `assets/diagrams/fyp-report/work-breakdown-structure.svg` are the deterministic editable and View-mode Figure 1.8 assets. The FYP DOCX remains external and is never rewritten by the Explorer generator.
...
- `scripts\generate-fyp-work-breakdown-structure.py`: generates the 4:3 FYP Work Breakdown Structure with 27 stable selectable components, 26 hierarchy connections, one embedded PetaKerja mark and bilingual Simple/Code metadata.
...
```powershell
python .\scripts\apply-user-explore-flowchart-default.py
python .\scripts\apply-user-explore-flowchart-default.py --adopt "C:\path\to\reviewed-map-flow.drawio"
```
...
- `scripts/test-bilingual-diagram-editor.py` inventories the workspace manifest and protects the all-diagram contract: 62 registered diagrams, 61 source files, complete BM/EN fields and unchanged geometry, styles and connector endpoints.
