...
- BM/EN and Ringkas/Kod changes use the same safe reload handshake. Before reload, the bridge commits active text editing and returns the latest XML, page, selection and scale. The controller canonicalises the bilingual metadata, then reloads the requested projection and restores the editor view.
