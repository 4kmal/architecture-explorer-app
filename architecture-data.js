(function () {
  ...
  const nodeList = [
    ...
    {
      id: 'daily-index-scraper', label: 'Daily Index Scraper', kind: 'service', scope: 'jobops', status: 'current',
      description: 'Larian TypeScript berjadual yang mengambil lapan sumber kerja, menormal dan menyahpendua hasil, kemudian mengupsert public.scraped_jobs menggunakan service role yang terhad kepada GitHub Actions.',
      files: source('.github/workflows/scrape-jobs.yml', 'scripts/scrape-jobs.ts'), tables: ['scraped_jobs'],
      flow: ['02:00 UTC atau workflow_dispatch', 'ambil lapan sumber', 'normalisasi + nyahpendua', 'upsert + pembersihan lapuk'],
    },
    {
      id: 'daily-index-job-sources', label: 'Daily Index Job Sources', kind: 'external', scope: 'jobops', status: 'current',
      description: 'Maukerja, Hiredly, Ricebowl, Graduan, Jora, JobStreet, Jobstore dan Careerjet yang dikonfigurasikan untuk larian Daily Index.',
    ...
    {
      id: 'wbs-project', label: 'PetaKerja FYP Project', kind: 'application', scope: 'core', status: 'current',
      description: 'Akar Struktur Pecahan Kerja Rajah 1.8 yang menyelaraskan lima aliran kerja Agile PetaKerja.',
      files: source('package.json', 'docs/architecture/overview.md'),
      flow: ['perancangan', 'reka bentuk', 'pembangunan', 'pengujian', 'dokumentasi dan penilaian'],
    },
    {
      id: 'wbs-planning-analysis', label: 'Planning and Analysis', kind: 'module', scope: 'core', status: 'current',
      description: 'Menetapkan masalah, objektif, skop, keperluan, sumber, risiko dan jadual lelaran sebelum pelaksanaan.',
      files: source('docs/architecture/overview.md', 'apps/architecture-explorer/fyp-report-tables.js', 'src/config/job-sources.ts', 'docs/planning/', 'package.json'),
    },
    {
      id: 'wbs-system-design', label: 'System Design', kind: 'module', scope: 'core', status: 'current',
      description: 'Menterjemah keperluan kepada seni bina, model data, UI/UX dan aliran kerja yang boleh dibina.',
      files: source('src/MyPetaApp.ts', 'server/app.ts', 'supabase/migrations/', 'src/services/supabase.ts', 'src/ui/templates.ts'),
    },
    {
      id: 'wbs-system-development', label: 'System Development', kind: 'module', scope: 'core', status: 'current',
      description: 'Melaksanakan modul peta, pekerjaan, POI, akaun dan integrasi pelayan PetaKerja.',
      files: source('src/', 'server/', 'supabase/'), routes: ['/api/*'],
    },
    {
      id: 'wbs-testing-improvement', label: 'Testing and Improvement', kind: 'module', scope: 'core', status: 'current',
      description: 'Menggabungkan semakan automatik, fungsi, aliran pengguna, prestasi, keselamatan dan kegagalan dengan pembaikan berdasarkan maklum balas.',
      files: source('package.json', 'tests/', 'scripts/check-bundle-budget.mjs', 'server/middleware/requireAuth.ts', 'docs/changelogs/'),
    },
    {
      id: 'wbs-documentation-evaluation', label: 'Documentation and Evaluation', kind: 'module', scope: 'core', status: 'current',
      description: 'Menyediakan dokumentasi teknikal, laporan FYP, penilaian objektif, pembentangan dan penyerahan akhir.',
      files: source('docs/', 'apps/architecture-explorer/README.md', 'docs-site/', 'vercel.json'),
    },
  ...
  const edges = [
    ...
    edge('wbs-project', 'wbs-planning-analysis', 'aliran kerja', ['fyp-work-breakdown-structure'], 'composition'),
    edge('wbs-project', 'wbs-system-design', 'aliran kerja', ['fyp-work-breakdown-structure'], 'composition'),
    edge('wbs-project', 'wbs-system-development', 'aliran kerja', ['fyp-work-breakdown-structure'], 'composition'),
    edge('wbs-project', 'wbs-testing-improvement', 'aliran kerja', ['fyp-work-breakdown-structure'], 'composition'),
    edge('wbs-project', 'wbs-documentation-evaluation', 'aliran kerja', ['fyp-work-breakdown-structure'], 'composition'),
    edge('wbs-system-design', 'mypeta-app', 'seni bina aplikasi', ['fyp-work-breakdown-structure'], 'dependency'),
    edge('wbs-system-design', 'supabase-db', 'reka bentuk data', ['fyp-work-breakdown-structure'], 'dependency'),
    edge('wbs-system-development', 'map-manager', 'peta 2D / 3D', ['fyp-work-breakdown-structure'], 'dependency'),
    edge('wbs-system-development', 'job-manager', 'carian pekerjaan', ['fyp-work-breakdown-structure'], 'dependency'),
    edge('wbs-system-development', 'geo-gateway', 'integrasi penghalaan', ['fyp-work-breakdown-structure'], 'dependency'),
    edge('wbs-testing-improvement', 'auth-manager', 'semakan keselamatan', ['fyp-work-breakdown-structure'], 'dependency'),
    ...
    edge('job-manager', 'jobs-api', 'search', ['overview', 'implementation', 'sequence', 'activity', 'data-flow', 'user-search-jobs-flowchart', 'daily-index-workflow'], 'dependency'),
    ...
    edge('jobs-api', 'supa-jobs-route', 'GET /api/jobs/supa', ['sequence', 'user-search-jobs-flowchart', 'daily-index-workflow'], 'dependency'),
    edge('supa-jobs-route', 'supabase-db', 'SELECT public.scraped_jobs', ['sequence', 'user-search-jobs-flowchart', 'daily-index-workflow'], 'dependency'),
    ...
    edge('github-actions', 'daily-index-scraper', 'daily 02:00 UTC or manual', ['daily-index-workflow'], 'dependency'),
    edge('daily-index-job-sources', 'daily-index-scraper', 'public listings and API results', ['daily-index-workflow'], 'dependency'),
  ...
  const diagrams = [
    ...
    },
    {
      ...
      columns: [
        ['github-actions', 'daily-index-job-sources'],
        ['daily-index-scraper'],
        ['supabase-db'],
...
Notes:
    ...
    {
      id: 'fyp-work-breakdown-structure', title: 'Struktur Pecahan Kerja', category: 'Laporan FYP', status: 'current', reference: 'assets/diagrams/fyp-report/work-breakdown-structure.svg',
      description: 'Rajah 1.8 yang memecahkan projek PetaKerja kepada lima aliran kerja Agile dan 21 pakej kerja, dengan label laporan atau bukti repositori.',
      collectionId: 'fyp-report', collectionGroupId: 'project-planning', collectionOrder: 1,
      reportExplanation: {
        en: 'Figure 1.8 divides the PetaKerja project into five coordinated Agile workstreams: planning and analysis establish the problem, objectives, scope, requirements, source constraints, risks and iteration schedule; design translates them into the layered architecture, Supabase/PostGIS data, UI/UX and job, POI and routing flows; development implements the application modules and integrations; testing combines automated, functional, user-flow, performance, security and failure checks with feedback-driven fixes; and documentation and evaluation prepare the technical material, final-year report, objective assessment, presentation and handover. The branches are a work breakdown rather than a strict waterfall sequence, so Agile feedback may return completed work to planning, design or development.',
        ms: 'Rajah 1.8 memecahkan projek PetaKerja kepada lima aliran kerja Agile yang saling berkaitan: perancangan dan analisis menetapkan masalah, objektif, skop, keperluan, kekangan sumber, risiko dan jadual lelaran; reka bentuk menterjemahkannya kepada seni bina berlapis, data Supabase/PostGIS, UI/UX serta aliran pekerjaan, POI dan penghalaan; pembangunan melaksanakan modul dan integrasi aplikasi; pengujian menggabungkan semakan automatik, fungsi, aliran pengguna, prestasi, keselamatan dan kegagalan dengan pembaikan berdasarkan maklum balas; manakala dokumentasi dan penilaian menyediakan bahan teknikal, laporan projek tahun akhir, penilaian objektif, pembentangan dan penyerahan. Cabang ini ialah struktur pecahan kerja dan bukannya urutan waterfall yang ketat, maka maklum balas Agile boleh mengembalikan kerja kepada fasa perancangan, reka bentuk atau pembangunan.',
      },
      columns: [
        ['wbs-project'],
        ['wbs-planning-analysis', 'wbs-system-design', 'wbs-system-development', 'wbs-testing-improvement', 'wbs-documentation-evaluation'],
        ['mypeta-app', 'supabase-db', 'map-manager', 'job-manager', 'geo-gateway', 'auth-manager'],
      ],
    },
    {
      id: 'fyp-kamus-data', title: 'Kamus Data', category: 'Laporan FYP', status: 'current', reference: null,
      ...
      collectionId: 'fyp-report', collectionGroupId: 'report-tables', collectionOrder: 2,
      layout: 'report-table', reportTableKey: 'kamus-data', columns: [],
    },
    {
      id: 'fyp-use-case-specification', title: 'Spesifikasi Kes Guna', category: 'Laporan FYP', status: 'current', reference: null,
      description: 'Lima belas jadual spesifikasi kes guna Bahasa Melayu berdasarkan aktor dan hubungan rajah semasa.',
      collectionId: 'fyp-report', collectionGroupId: 'report-tables', collectionOrder: 3,
...
watchlist_polls.user_id>users.id`.trim();
  ...
  window.PETAKERJA_ARCHITECTURE = {
    ...
    sourceRefs: {
      ...
      fypWorkBreakdownStructure: 'assets/editor/fyp-report/work-breakdown-structure.drawio',
